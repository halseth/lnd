...
func TestSignatureAnnouncementLocalFirst(t *testing.T) {
	...
	ctx.gossiper.cfg.NotifyWhenOnline = func(target *btcec.PublicKey,
		peerChan chan<- lnpeer.Peer) {
...
func TestSignatureAnnouncementRetryAtStartup(t *testing.T) {
	...
	ctx.gossiper.cfg.NotifyWhenOffline = func(_ [33]byte) <-chan struct{} {
		c := make(chan struct{})
		notifyOffline <- c
		return c
	}
	...
	gossiper := New(Config{
		...
		NotifyWhenOnline:  ctx.gossiper.cfg.NotifyWhenOnline,
		NotifyWhenOffline: ctx.gossiper.cfg.NotifyWhenOffline,
