...
// GetStatus returns the current active status of the autopilot agent.
func (r *Manager) GetStatus() bool {
	r.Lock()
	defer r.Unlock()
...
// config.
func (r *Manager) StartAgent() error {
	r.Lock()
	defer r.Unlock()
...
func (r *Manager) StopAgent() error {
	r.Lock()
	defer r.Unlock()
