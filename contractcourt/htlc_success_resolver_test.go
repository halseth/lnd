	...
	checkpoints := []checkpoint{
		{
			...
			// checkkpoint to be resolved, and with the above
			// report.
			resolved: true,
...
func TestHtlcSuccessSecondStageResolution(t *testing.T) {
	...
	checkpoints := []checkpoint{
		{
			// The resolver will send the ouput to the Nursery.
			incubating: true,
		},
		{
			// It will then wait for the Nursery to spend the
			// ouput. We send a spend notification for our output
			// to resolve our htlc.
			preCheckpoint: func(ctx *htlcSuccessResolverTestContext,
				_ bool) error {
	...
	checkpoints := []checkpoint{
		{
			...
			// incubating=true is used to signat that the
		...
		},
		{
			...
			preCheckpoint: func(ctx *htlcSuccessResolverTestContext,
				...
				inp := <-ctx.resolver.Sweeper.(*mockSweeper).
					sweptInputs
				op := inp.OutPoint()
				exp := wire.OutPoint{reSignedHash, 1}
				if *op != exp {
...
// resolver all the way through these checkpoints, and also attemp to resume
