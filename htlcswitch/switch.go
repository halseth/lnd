...
func (s *Switch) extractResult(deobfuscator ErrorDecrypter,
	n *networkResult, paymentHash lntypes.Hash) (*PaymentResult, error) {
	...
	case *lnwire.UpdateFulfillHTLC:
		...
		if err != nil && err != ErrPaymentAlreadyCompleted {
			return nil, fmt.Errorf("Unable to mark completed payment %x: %v",
				paymentHash, err)
		}
	...
	case *lnwire.UpdateFailHTLC:
		...
		if err != nil && err != ErrPaymentAlreadyCompleted {
			return nil, fmt.Errorf("Unable to ground payment %x: %v",
				paymentHash, err)
		}
	...
	default:
		return nil, fmt.Errorf("Received unknown response type: %T", htlc)
	}
}
