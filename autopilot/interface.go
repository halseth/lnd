...
type AttachmentHeuristic interface {
	...
	// returned channel candidates maps the NodeID to a attachemnt
	// directive containing a score and a channel size. The scores will be
	// in the range [0, M], where 0 indicates no improvement in
	// connectivity if a channel is opened to this node, while M is the
	// maximum possible improvement in connectivity.
