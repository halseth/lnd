...
func (p *paymentLifecycle) resumePayment() ([32]byte, *route.Route, error) {
	...
	for {
		...
		var (
			...
			// If the payment is already marked failed, we'll get
		...
		// We ew encountered a non-critical error when launching the
		// shard, handle it
		if outcome.err != nil {
