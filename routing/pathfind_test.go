...
func TestFindLowestFeePath(t *testing.T) {
	...
	path, err := findPath(
		&findPathParams{
			...
			ignoredVertexes, ignoredEdges, paymentAmt, noFeeLimit,
			nil,
		},
	)
	if err != nil {
...
func testBasicGraphPathFindingCase(t *testing.T, graphInstance *testGraphInstance,
	...
	path, err := findPath(
		&findPathParams{
			...
			ignoredVertexes, ignoredEdges, paymentAmt,
			test.feeLimit, nil,
		},
	)
...
func TestPathFindingWithAdditionalEdges(t *testing.T) {
	...
	path, err := findPath(
		&findPathParams{
			nil, graph.graph, additionalEdges, sourceNode,
			dogePubKey, nil, nil, paymentAmt, noFeeLimit, nil,
		},
	)
	if err != nil {
...
func TestNewRoutePathTooLong(t *testing.T) {
	...
	_, err = findPath(
		&findPathParams{
			nil, graph.graph, nil, sourceNode, target,
			ignoredVertexes, ignoredEdges, paymentAmt, noFeeLimit,
			nil,
		},
	)
...
func TestPathNotAvailable(t *testing.T) {
	...
	_, err = findPath(
		&findPathParams{
			nil, graph.graph, nil, sourceNode, unknownNode,
...
func TestPathInsufficientCapacity(t *testing.T) {
	...
	_, err = findPath(
		&findPathParams{
			nil, graph.graph, nil, sourceNode, target,
			ignoredVertexes, ignoredEdges, payAmt, noFeeLimit, nil,
		},
	)
	if !IsError(err, ErrNoPathFound) {
...
func TestPathSourceEdgesBandwidth(t *testing.T) {
	...
	_, err = findPath(
		&findPathParams{
			nil, graph.graph, nil, sourceNode, target,
			ignoredVertexes, ignoredEdges, payAmt, noFeeLimit,
			bandwidths,
		},
	)
	if !IsError(err, ErrNoPathFound) {
