...
// fee is a helper method that returns the fee estimate used for a tx with the
// given number of inputs and the optional change output. This matches the
// estimate done by the wallet.
func fee(feeRate SatPerKWeight, numInput int, change bool) btcutil.Amount {
	...
	testCases := []testCase{
		{
			...
			expectedChange: 0.5*btcutil.SatoshiPerBitcoin - fee(feeRate, 1, true),
		},
		{
		...
		},
		{
			...
			outputValue: 1*btcutil.SatoshiPerBitcoin - fee(feeRate, 1, true) - dust,
		...
		},
		{
			...
			outputValue: 1*btcutil.SatoshiPerBitcoin - fee(feeRate, 1, true),
...
// TestCoinSelectSubtractFees tests that we pick coins adding up to the expected amount
	...
	testCases := []testCase{
		{
			...
			expectedFundingAmt: 1*btcutil.SatoshiPerBitcoin - fee(feeRate, 1, false),
			expectedChange:     0,
		},
		{
			...
			coins: []*Utxo{
				{
					AddressType: WitnessPubKey,
					Value:       fee(feeRate, 1, false) + dust,
				},
			},
			spendValue: fee(feeRate, 1, false) + dust,
		...
		},
		{
			...
			coins: []*Utxo{
				{
					AddressType: WitnessPubKey,
					Value:       fee(feeRate, 1, false) + dustLimit + 1,
				},
			},
			spendValue: fee(feeRate, 1, false) + dustLimit + 1,

			expectedInput: []btcutil.Amount{
				fee(feeRate, 1, false) + dustLimit + 1,
			},
		...
		},
		{
			...
			coins: []*Utxo{
				{
					AddressType: WitnessPubKey,
					Value:       fee(feeRate, 1, false) + 2*(dustLimit+1),
				},
			},
			spendValue: fee(feeRate, 1, false) + dustLimit + 1,

			expectedInput: []btcutil.Amount{
				fee(feeRate, 1, false) + 2*(dustLimit+1),
			},
		...
		},
		{
			...
			coins: []*Utxo{
				{
					AddressType: WitnessPubKey,
					Value:       5 * fee(feeRate, 1, false),
				},
			},
			spendValue: 5 * fee(feeRate, 1, false),
