...
func (h *htlcTimeoutResolver) waitForSpend(op *wire.OutPoint,
...
func (h *htlcTimeoutResolver) handleCommitSpend(
	...
	case h.htlcResolution.SignDetails != nil:
		...
		// Find the input index of the second-level spend, as the
		// sweeper might have re-arranged inputs and outputs.
		prevOutPoint := h.htlcResolution.SignedTimeoutTx.TxIn[0].PreviousOutPoint
		ok, index := input.FindInputIndex(
			commitSpend.SpendingTx, prevOutPoint,
		)
		if !ok {
			err := fmt.Errorf("unable to find second-level "+
				"input %v on spending tx %v", prevOutPoint,
