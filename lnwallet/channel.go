...
func (lc *LightningChannel) diskCommitToMemCommit(isLocal bool,
	...
	if localCommitPoint != nil {
		localCommitKeys = DeriveCommitmentKeys(
			...
			&lc.channelState.OurChanCfg, &lc.channelState.TheirChanCfg,
		)
	}
...
func (lc *LightningChannel) createSignDesc() error {
	localKey := lc.channelState.OurChanCfg.MultiSigKey.PubKey.SerializeCompressed()
	remoteKey := lc.channelState.TheirChanCfg.MultiSigKey.PubKey.SerializeCompressed()
	...
	lc.signDesc = &input.SignDescriptor{
		KeyDesc:       lc.channelState.OurChanCfg.MultiSigKey,
...
func (lc *LightningChannel) fetchCommitmentView(remoteChain bool,
	...
	if remoteChain {
		dustLimit = lc.channelState.TheirChanCfg.DustLimit
	} else {
		dustLimit = lc.channelState.OurChanCfg.DustLimit
	}
...
func (lc *LightningChannel) createCommitmentTx(c *commitment,
	...
	if c.isOurs {
		localCfg = &lc.channelState.OurChanCfg
		remoteCfg = &lc.channelState.TheirChanCfg
		localBalance = ourBalance.ToSatoshis()
		remoteBalance = theirBalance.ToSatoshis()
	} else {
		localCfg = &lc.channelState.TheirChanCfg
		remoteCfg = &lc.channelState.OurChanCfg
...
func (lc *LightningChannel) validateCommitmentSanity(theirLogCounter,
	...
	case theirBalance < theirInitialBalance &&
		theirBalance < lnwire.NewMSatFromSatoshis(
			lc.channelState.TheirChanCfg.ChanReserve):
	...
	// constraints.
	err := validateUpdates(
		filteredView.theirUpdates, &lc.channelState.TheirChanCfg,
	)
	if err != nil {
		return err
	}
	...
	// constraints.
	err = validateUpdates(
		filteredView.ourUpdates, &lc.channelState.OurChanCfg,
	)
	if err != nil {
		return err
	}
...
func (lc *LightningChannel) SignNextCommitment() (lnwire.Sig, []lnwire.Sig, []channeldb.HTLC, error) {
	...
	sigBatch, cancelChan, err := genRemoteHtlcSigJobs(keyRing,
		&lc.channelState.OurChanCfg, &lc.channelState.TheirChanCfg, newCommitView,
	)
	if err != nil {
		return sig, htlcSigs, nil, err
	}
...
func (lc *LightningChannel) ReceiveNewCommitment(commitSig lnwire.Sig,
	...
	verifyJobs, err := genHtlcSigValidationJobs(
		localCommitmentView, keyRing, htlcSigs, &lc.channelState.OurChanCfg,
		&lc.channelState.TheirChanCfg,
	)
	if err != nil {
		return err
	}
	...
	verifyKey := btcec.PublicKey{
		X:     lc.channelState.TheirChanCfg.MultiSigKey.PubKey.X,
		Y:     lc.channelState.TheirChanCfg.MultiSigKey.PubKey.Y,
		Curve: btcec.S256(),
	}
...
func (lc *LightningChannel) getSignedCommitTx() (*wire.MsgTx, error) {
	...
	ourKey := lc.channelState.OurChanCfg.MultiSigKey.PubKey.SerializeCompressed()
	theirKey := lc.channelState.TheirChanCfg.MultiSigKey.PubKey.SerializeCompressed()
...
func NewUnilateralCloseSummary(chanState *channeldb.OpenChannel, signer input.Signer,
	...
	closeSummary := channeldb.ChannelCloseSummary{
		...
		OurChanConfig:           chanState.OurChanCfg,
	}
...
func (lc *LightningChannel) CreateCloseProposal(proposedFee btcutil.Amount,
	...
	closeTx := CreateCooperativeCloseTx(
		lc.fundingTxIn(), lc.channelState.OurChanCfg.DustLimit,
		lc.channelState.TheirChanCfg.DustLimit, ourBalance, theirBalance,
...
func (lc *LightningChannel) LocalChanReserve() btcutil.Amount {
	return lc.channelState.OurChanCfg.ChanReserve
}
...
func (lc *LightningChannel) FwdMinHtlc() lnwire.MilliSatoshi {
	return lc.channelState.OurChanCfg.MinHTLC
}
