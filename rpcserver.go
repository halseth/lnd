...
func (r *rpcServer) PendingChannels(ctx context.Context,
	...
	for i, pendingChan := range pendingOpenChannels {
		...
		resp.PendingOpenChannels[i] = &lnrpc.PendingChannelsResponse_PendingOpenChannel{
			Channel: &lnrpc.PendingChannelsResponse_PendingChannel{
				...
				LocalChanReserveSat:  int64(pendingChan.OurChanCfg.ChanReserve),
				RemoteChanReserveSat: int64(pendingChan.TheirChanCfg.ChanReserve),
			},
	...
	for _, waitingClose := range waitingCloseChans {
		...
		channel := &lnrpc.PendingChannelsResponse_PendingChannel{
			...
			LocalChanReserveSat:  int64(waitingClose.OurChanCfg.ChanReserve),
			RemoteChanReserveSat: int64(waitingClose.TheirChanCfg.ChanReserve),
		}
...
func createRPCOpenChannel(r *rpcServer, graph *channeldb.ChannelGraph,
	...
	channel := &lnrpc.Channel{
		...
		CsvDelay:              uint32(dbChannel.OurChanCfg.CsvDelay),
		Initiator:             dbChannel.IsInitiator,
		ChanStatusFlags:       dbChannel.ChanStatus().String(),
		LocalChanReserveSat:   int64(dbChannel.OurChanCfg.ChanReserve),
		RemoteChanReserveSat:  int64(dbChannel.TheirChanCfg.ChanReserve),
