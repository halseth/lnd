...
func TestChannelLinkMultiHopUnknownPaymentHash(t *testing.T) {
	...
	result, err := n.aliceServer.htlcSwitch.GetPaymentResult(pid)
	if err != nil {
		t.Fatalf("unable to get payment result: %v", err)
	}

	failure, ok := result.(*PaymentFailure)
	if !ok {
		t.Fatalf("expected failure, got %T", result)
	}
	fErr := failure.Error
...
func TestChannelLinkAcceptDuplicatePayment(t *testing.T) {
	...
	_, err = n.aliceServer.htlcSwitch.GetPaymentResult(pid)
	if err != nil {
		t.Fatalf("unable to get payment result: %v", err)
	}
