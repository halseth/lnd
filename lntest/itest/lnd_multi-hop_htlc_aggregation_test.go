...
func testMultiHopHtlcAggregation(net *lntest.NetworkHarness, t *harnessTest,
	...
	var (
		...
		carolPreimages []lntypes.Preimage
	...
	for i := 0; i < numInvoices; i++ {
		...
		carolPreimages = append(carolPreimages, preimage)
		payHashes = append(payHashes, payHash[:])
		carolPayHashes = append(carolPayHashes, payHash[:])
	}

	// We'll give Alice's invoices a longeger CLTV expiry, to ensure the
	...
	for i, txOut := range closeTx.TxOut {
		...
		// claim it with the success TX when he learns the
		// preimage. In this case one extra sat will be on the output, because of the routing fee.
		case invoiceAmt + 1:
