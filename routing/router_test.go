...
func failOnPid(pid uint64, errSource *btcec.PublicKey, failure lnwire.FailureMessage,
...
func failAlways(errSource *btcec.PublicKey, failure lnwire.FailureMessage) func(uint64) (
	<-chan *htlcswitch.PaymentResult, error) {
...
func TestSendPaymentRouteFailureFallback(t *testing.T) {
	...
	ctx.router.cfg.GetPaymentResult = failOnPid(pid, pub, &lnwire.FailTemporaryChannelFailure{}, preImage)
...
func TestChannelUpdateValidation(t *testing.T) {
	...
	ctx.router.cfg.GetPaymentResult = failAlways(
		source, &lnwire.FailFeeInsufficient{
...
func TestSendPaymentErrorNonFinalTimeLockErrors(t *testing.T) {
	...
	ctx.router.cfg.GetPaymentResult = failOnPid(pid, sourceKey,
...
func TestRouterPaymentStateMachine(t *testing.T) {
	...
	const (
		routerMakeFirstAttempt  = "Router:first-attempt"
		routerMakeSecondAttempt = "Router:second-attempt"
	...
	setupRouter := func() (*ChannelRouter, chan error,
		...
		// We use the following channels to synchonize with the
		// interaction to the Switch.
		...
		router, err := New(Config{
			...
			GetPaymentResult: func(paymentID uint64) (
				*htlcswitch.PaymentResult, error) {

				select {
				case res := <-paymentResult:
					return res, nil
				case err := <-paymentResultErr:
					return nil, err
				case <-quit:
	...
	for _, test := range tests {
		...
		go func() {
			// We copy the test routes, since the PaymentSession
			// will modify the slice.
			...
			// TODO(halseth): make SendPayment instead, by mocking
			// MissionControl/PaymentSession.
			_, _, err := router.SendToRoute(routes, &payment)
			paymentResult <- err
		}()
		...
		for _, step := range test.steps {
			...
			case routerMakeSecondAttempt:
				if firstAttempt {
					t.Fatalf("first attempt not done")
				}
				...
				if args.c != nil {
					t.Fatalf("expected nil CreationInfo")
				}
			...
			case routerSuccess:
				...
				var args successArgs
				select {
				case args = <-control.success:
				case <-time.After(1 * time.Second):
					t.Fatalf("not registered with control")
				}
			...
			case routerFail:
				var args failArgs
				select {
				case args = <-control.fail:
				case <-time.After(1 * time.Second):
					t.Fatalf("not registered with control")
				}

				// It could fail immediately, if so the
				// CreationInfo should be supplied.
				if firstAttempt {
					if args.c == nil {
			...
			case resendPayment:
				// We copy the test routes, since the
				// PaymentSession will modify the slice.
				var routes []*route.Route
				for _, r := range test.routes {
					c := *r
					routes = append(routes, &c)
...
type registerArgs struct {
	c *channeldb.CreationInfo
	a *channeldb.AttemptInfo
}
...
func (m *mockControlTower) init(phash lntypes.Hash,
	c *channeldb.CreationInfo) error {
...
func (m *mockControlTower) RegisterAttempt(phash lntypes.Hash,
	c *channeldb.CreationInfo, a *channeldb.AttemptInfo) error {

	m.Lock()
	defer m.Unlock()

	if m.register != nil {
		m.register <- registerArgs{c, a}
	}

	if err := m.init(phash, c); err != nil {
		return err
	}
...
func (m *mockControlTower) Success(phash lntypes.Hash,
	c *channeldb.CreationInfo, preimg lntypes.Preimage) error {

	m.Lock()
	defer m.Unlock()

	if m.success != nil {
		m.success <- successArgs{c, preimg}
	}
