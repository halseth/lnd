...
func (s *commitmentChain) addCommitment(c *types.Commitment) {
...
func (s *commitmentChain) tip() *types.Commitment {
	return s.commitments.Back().Value.(*types.Commitment)
}

// tail returns the lowest unrevoked commitment transaction in the chain.
func (s *commitmentChain) tail() *types.Commitment {
	return s.commitments.Front().Value.(*types.Commitment)
}
...
func (lc *LightningChannel) logUpdateToPayDesc(logUpdate *channeldb.LogUpdate,
	...
	case *lnwire.UpdateAddHTLC:
		...
		if !isDustRemote {
			theirP2WSH, theirWitnessScript, err := types.GenHtlcScript(
...
func (lc *LightningChannel) restoreCommitState(
	...
	localCommit, err := types.DiskCommitToMemCommit(
		lc.commitType,
		true,
		&lc.channelState.LocalChanCfg,
		&lc.channelState.RemoteChanCfg,
		lc.channelState,
		localCommitState, localCommitPoint,
		remoteCommitPoint,
	)
	if err != nil {
		return err
	...
	remoteCommit, err := types.DiskCommitToMemCommit(
		lc.commitType,
		false,
		&lc.channelState.LocalChanCfg,
		&lc.channelState.RemoteChanCfg,
		lc.channelState,
		remoteCommitState, localCommitPoint,
		remoteCommitPoint,
	)
	if err != nil {
		return err
	...
	var (
		pendingRemoteCommit     *types.Commitment
	...
	if pendingRemoteCommitDiff != nil {
		...
		pendingRemoteCommit, err = types.DiskCommitToMemCommit(
			lc.commitType,
			false,
			&lc.channelState.LocalChanCfg,
			&lc.channelState.RemoteChanCfg,
			lc.channelState,
			&pendingRemoteCommitDiff.Commitment,
...
func (lc *LightningChannel) restoreStateLogs(
	localCommitment, remoteCommitment, pendingRemoteCommit *types.Commitment,
	pendingRemoteCommitDiff *channeldb.CommitDiff,
...
func (lc *LightningChannel) fetchCommitmentView(remoteChain bool,
	...
	keyRing *commitmenttx.KeyRing) (*types.Commitment, error) {
...
func genRemoteHtlcSigJobs(keyRing *commitmenttx.KeyRing,
	...
	remoteCommitView *types.Commitment) ([]SignJob, chan struct{}, error) {
...
func (lc *LightningChannel) createCommitDiff(
	newCommit *types.Commitment, commitSig lnwire.Sig,
...
func genHtlcSigValidationJobs(localCommitmentView *types.Commitment,
