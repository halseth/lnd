...
func TestChanAvailableBalanceNearHtlcFee(t *testing.T) {
	...
	// for Bob adding another HTLC. This means shea actualy _has_ some
	// balance left, and we now force the channel into this situation by
	// sending yet another HTLC. In practice this can also happend if a fee
...
func TestChanReserveRemoteInitiator(t *testing.T) {
	...
	// HTLCs on the commitment transaction.
	htlcAmt := lnwire.NewMSatFromSatoshis(0.1 * btcutil.SatoshiPerBitcoin)
	htlc, _ := createHTLC(0, htlcAmt)
	...
	// Same for Alice, she should refuse to accept a state transtion with
	// this second HTLC.
	if _, err := aliceChannel.ReceiveHTLC(htlc); err != nil {
		t.Fatalf("unable to recv htlc: %v", err)
	}
...
// when adding HTLCs is ccounted for, even though the HTLC is considered dust
// by the remote bode.
