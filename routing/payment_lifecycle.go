...
func (p *paymentLifecycle) paymentState(payment *channeldb.MPPayment) (
	...
	var (
		...
		// We calculate whether we have any fees left in our budget
		// after subtracting the fees from active shards. Note that
		// currently we set the fee limit to zero after a restart, so
		// the active shards might be using more than the limit in that
		// case.
		feeBudget = p.feeLimit

		// If the payment is already marked failed, we'll get ready to
		// terminate the moment all shards have returned.
...
func (p *paymentLifecycle) resumePayment() ([32]byte, *route.Route, error) {
	...
	for {
		...
		activeShards, remValue, feeLimit, terminate, err := p.paymentState(
			payment,
		)
		if err != nil {
			return [32]byte{}, nil, err
		...
		log.Debugf("Payment %v in state terminate=%v, "+
			...
			p.paymentHash, terminate, activeShards, remValue,
			feeLimit)
...
func (p *shardHandler) collectResultAsync(attempt *channeldb.HTLCAttemptInfo) {
	...
	go func() {
		...
		if err != nil {
			log.Errorf("Error collecting result for shard %v for "+
				"payment %v: %v", attempt.AttemptID,
				p.paymentHash, err)
