...
func TestSwitchSendDuplicatePayment(t *testing.T) {
	...
	// Create a DB that we will use across restarts.
	db, err := initDB()
	if err != nil {
		t.Fatal(err)
	}

	// Create and start the switch with the DB.
	s, err := initSwitchWithDB(testStartingHeight, db)
	if err != nil {
		t.Fatalf("unable to init switch: %v", err)
	}
	// For now we use a mock ControlTower to allow payment hash reuses.
	s.control = &mockControlTower{}
...
func TestSwitchSendDuplicateSettledPayment(t *testing.T) {
	...
	// We'll setup a preimage cache having a "lookup delay" of 100ms. This
	// is used to delay the switch after it looks up a payment hash in the
	// preimage cache, such that we can trigger the situation where the
	// preimage is added immediately after lookup, but before the HTLC is
	// forwarded.
	pCache := &mockPreimageCache{
		preimageMap: make(map[[32]byte][]byte),
	...
	// Concurrently with re-sending the payment we'll fulfill it. This
	// triggers a race condition where the fulfill comes in after the
	// re-sent payment has finished preimage lookup, but _before_ the HTLC
	// is forwarded. We trigger this situation by the "lookup delay" we sat
	// above, which makes the switch wait a bit after preimage lookup, but
	// before re-forwarding. This gives the Settle enough time to tear down
	// the circuit and attempt to deliver the preimage before the switch
	// calls forward again.
	packet := &htlcPacket{
		outgoingChanID: aliceChannelLink.ShortChanID(),
		outgoingHTLCID: 0,
	...
	// And we should reciev the preimage on the preimg channel.
	select {
	case pre := <-preChan:
		if pre != preimage {
			t.Fatalf("received incorrect preimage")
...
// failed, the caller will receive the failure.
func TestSwitchSendDuplicateFailedPayment(t *testing.T) {
	...
	// We set up a mock preimage cach such that we can synchronize when the
	// payments get sent.
	pCache := &mockPreimageCache{
		preimageMap: make(map[[32]byte][]byte),
	}
	...
	// Before resending the payment, we set up a channel we can use to know
	// when the switch has received the LookupPreimage phase.
	pCache.lookupEnter = make(chan struct{})
	...
	// Concurrently with re-sending the payment we'll fail it. This
	// triggers a race condition where the failure comes in and tears down
	// the circuit before the re-sent payment is attempted re-forwarded. In
	// that case we risked that the HTLC was re-forwarded, but the caller
	// received an error from the previous failure.
	obfuscator := NewMockObfuscator()
	failure := lnwire.FailIncorrectPaymentAmount{}
	reason, err := obfuscator.EncryptFirstHop(failure)
	if err != nil {
	...
	// Before forwarding the failure, we'll wait for SendHTLC to have
	// entered the preimage lookup. We do this to make sure the failure
	// doesn't reach the switch before the goroutine calling SendHTLC has
	// started.
	select {
	case <-pCache.lookupEnter:
	case <-time.After(2 * time.Second):
		t.Fatalf("did not enter lookup")
	}
...
func TestSwitchSendHTLCUniquePaymentID(t *testing.T) {
	...
	// Send the HTLC using paymentID=0, and wait for it to be send to the
	// link such that it can be committed.
	errChan := make(chan error, 1)
	go func() {
		_, err := s.SendHTLC(
			aliceChannelLink.ShortChanID(), 0, update,
