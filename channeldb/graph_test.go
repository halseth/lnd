...
// executes mutliple AddChannelEdge requests in a single txn.
func TestBatchedAddChannelEdge(t *testing.T) {
	...
	for _, edge := range edges {
		...
		go func(edge ChannelEdgeInfo) {
			...
			select {
			case errChan <- graph.BatchedAddChannelEdge(&edge):
...
// executes mutliple UpdateEdgePolicy requests in a single txn.
func TestBatchedUpdateEdgePolicy(t *testing.T) {
	...
	// Make sure inserting the policy at this point, before the edge info
	// is added, will fail.
	err = graph.BatchedUpdateEdgePolicy(edge1)
	...
	for _, update := range updates {
		...
		go func(update *ChannelEdgePolicy) {
			...
			select {
			case errChan <- graph.BatchedUpdateEdgePolicy(update):
			case <-time.After(2 * time.Second):
				errChan <- errTimeout
			}
		}(update)
