...
const (
	...
	// CommitmentToRemoteDelayed is a witness that allows us to spend our
	// output on the counterparty's commitment transaction after a relative
	// lock-time lockout.
	CommitmentToRemoteDelayed StandardWitnessType = 13
)
...
func (wt StandardWitnessType) String() string {
	...
	case CommitmentToRemoteDelayed:
		return "CommitmentToRemoteDelayed"
...
func (wt StandardWitnessType) WitnessGenerator(signer Signer,
	...
	return func(tx *wire.MsgTx, hc *txscript.TxSigHashes,
		...
		case CommitmentToRemoteDelayed:
			witness, err := CommitSpendToRemoteDelayed(
				signer, desc, tx,
			)
			if err != nil {
				return nil, err
...
func (wt StandardWitnessType) SizeUpperBound() (int, bool, error) {
	...
	// Timelocked output to us on remote commitment.
	case CommitmentToRemoteDelayed:
		return ToRemoteDelayedWitnessSize, false, nil
