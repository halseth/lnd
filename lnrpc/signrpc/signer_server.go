...
func (s *Server) SignOutputRaw(ctx context.Context, in *SignReq) (*SignResp, error) {
	...
	for _, signDesc := range in.SignDescs {
		...
		var targetPubKey *btcec.PublicKey
		switch {

		// If the user provided a raw key, but it's of the wrong
		// length, then we'll return with an error.
		case len(keyDesc.RawKeyBytes) != 0 && len(keyDesc.RawKeyBytes) != 33:
			return nil, fmt.Errorf("pubkey must be serialized in " +
				"compressed format if specified")

		// If a proper raw key was provided, then we'll attempt to
		// decode and parse it.
		case len(keyDesc.RawKeyBytes) != 0 && len(keyDesc.RawKeyBytes) == 33:
			targetPubKey, err = btcec.ParsePubKey(
				keyDesc.RawKeyBytes, btcec.S256(),
			)
			if err != nil {
			...
			KeyDesc: keychain.KeyDescriptor{
				KeyLocator: keychain.KeyLocator{
					Family: keychain.KeyFamily(
						keyDesc.KeyLoc.KeyFamily,
					),
					Index: uint32(keyDesc.KeyLoc.KeyIndex),
				},
				PubKey: targetPubKey,
			},
