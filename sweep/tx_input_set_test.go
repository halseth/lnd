...
func TestTxInputSetRequiredOutput(t *testing.T) {
	...
	// Finally we add an input that should push the change output above the dust limit.
	weight = set.weightEstimate(true)
	weight.add(dummyInput)
	fee = weight.fee()

	// We expect the change to everything that is left after paying the tx fee,
