...
func (r *rpcServer) ListPayments(ctx context.Context,
	...
	payments, err := r.server.chanDB.FetchSentPayments()
	if err != nil {
		return nil, err
	}
	...
	for i, payment := range payments {
		path := make([]string, len(payment.Route.Hops))
		for i, hop := range payment.Route.Hops {
		...
		paymentsResp.Payments[i] = &lnrpc.Payment{
			...
			Path:            path,
			Fee:             int64(payment.Route.TotalFees.ToSatoshis()),
...
func (r *rpcServer) DeleteAllPayments(ctx context.Context,
	...
	if err := r.server.chanDB.DeleteSentPayments(); err != nil {
		return nil, err
	}
